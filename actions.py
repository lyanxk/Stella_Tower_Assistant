...
def select_choice_or_first() -> None:
    ...
    if select_match:
        print(f"[Debug] select matched at {select_match}")
        ...
        while time.time() - start_time < SELECT_CONFIRM_TIMEOUT:
            ...
            if confirm_match:
                print(f"[Debug] select_confirm matched at {confirm_match}")
    ...
    if choice_match:
        print(f"[Debug] choice matched at {choice_match}")
    ...
    print(f"[Debug] fallback click at ({fallback_x}, {fallback_y})")
    state.check_pause_and_running()
